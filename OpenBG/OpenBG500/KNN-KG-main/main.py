...
def _setup_parser():
    """Set up Python's ArgumentParser with data, model, trainer, and other arguments."""
    # 创建基础的 ArgumentParser
    parser = argparse.ArgumentParser(add_help=False)

    # 创建一个新的 Trainer 参数组
    trainer_group = parser.add_argument_group("Trainer Args")
    trainer_group.add_argument("--max_epochs", type=int, default=10, help="Max number of epochs")
    trainer_group.add_argument("--gpus", type=int, default=1, help="Number of GPUs to use")
    trainer_group.add_argument("--precision", type=int, default=32, help="Precision (16, 32, or 64)")
...
def main():
    ...
    # 使用解析的参数初始化 Trainer
    trainer = Trainer(max_epochs=args.max_epochs, gpus=args.gpus, precision=args.precision)
