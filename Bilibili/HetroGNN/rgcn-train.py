...
def evaluate(g, target_idx, labels, test_mask, model):
    ...
    return accuracy(logits[test_idx].argmax(dim=1), labels[test_idx], task="multiclass", num_classes=4).item()
...
def train(g, target_idx, labels, train_mask, model):
    ...
    for epoch in range(50):
        ...
        loss.backward()
        optimizer.step()
        acc = accuracy(logits[train_idx].argmax(dim=1), labels[train_idx], task="multiclass", num_classes=4).item()
